...
def setup_scheduler():
    ...
    # Schedule tasks
    scheduler.add_job(games_live_update_task, IntervalTrigger(seconds=15))
    scheduler.add_job(fetch_odds_task, IntervalTrigger(minutes=1))
    scheduler.add_job(player_rotowires_task, IntervalTrigger(minutes=30))
    scheduler.add_job(reset_flags_task, CronTrigger(hour=0, minute=0))
    scheduler.add_job(daily_update_task, CronTrigger(hour=2, minute=0))
